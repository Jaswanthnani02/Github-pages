...
const observer = new IntersectionObserver((entries) => {
    entries.forEach(entry => {
        if (entry.isIntersecting) {
            // Enhanced fade-in with spring animation
            entry.target.style.opacity = '1';
            entry.target.style.transform = 'translateY(0) scale(1)';
            entry.target.style.transition = 'all 0.8s cubic-bezier(0.34, 1.56, 0.64, 1)';

            // Add staggered animation for child elements with smoother timing
            ...
            children.forEach((child, index) => {
                ...
                }, index * 100); // Smoother stagger delay
            });
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (photoContainer) {
        // Enhanced mouse move effect for photo with smoother response
        photoContainer.addEventListener('mousemove', (e) => {
            ...
            photoContainer.style.transform = `perspective(1000px) rotateX(${rotateX}deg) rotateY(${rotateY}deg) scale(1.08)`;
            photoContainer.style.transition = 'transform 0.1s ease-out';

            // Add enhanced parallax effect to floating icons
            floatingIcons.forEach((icon, index) => {
                const speed = (index + 1) * 0.8;
                const moveX = (x - centerX) * speed * 0.015;
                const moveY = (y - centerY) * speed * 0.015;
        ...
        // Enhanced click effect with bigger ripple and scale
        photoContainer.addEventListener('click', () => {
            ...
            ripple.style.cssText = `
                position: absolute;
                border-radius: 50%;
                background: radial-gradient(circle, rgba(139, 92, 246, 0.6), rgba(255, 255, 255, 0.2));
                transform: scale(0);
                animation: ripple-enhanced 0.8s cubic-bezier(0.4, 0, 0.2, 1);
                pointer-events: none;
            `;
            ...
            photoContainer.style.transform = 'perspective(1000px) rotateX(0deg) rotateY(0deg) scale(1.15)';
            photoContainer.style.transition = 'transform 0.3s cubic-bezier(0.34, 1.56, 0.64, 1)';
            setTimeout(() => {
    ...
    floatingIcons.forEach(icon => {
        icon.addEventListener('mouseenter', () => {
            ...
            icon.style.boxShadow = '0 0 40px rgba(139, 92, 246, 1), 0 0 60px rgba(96, 165, 250, 0.6)';
        });
        ...
        icon.addEventListener('click', () => {
            icon.style.transform = 'scale(1.6) rotate(720deg)';
            icon.style.transition = 'all 0.6s cubic-bezier(0.34, 1.56, 0.64, 1)';
            icon.style.boxShadow = '0 0 50px rgba(139, 92, 246, 1), 0 0 80px rgba(96, 165, 250, 0.8)';
